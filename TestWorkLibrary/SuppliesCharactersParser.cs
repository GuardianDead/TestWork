...
namespace TestWorkLibrary.Parser
{
    public static class SuppliesCharactersParser
    {
        ...
        public static void SaveFile(string fullPath, params Character[] characters)
        {
        ...
        public static void SaveInSameFile(string fullPath, params Character[] characters)
        {
            if (!File.Exists(fullPath))
                File.Create(fullPath).Close();
        ...
        private static Supplies CreateSupplies(MatchCollection matchedValues)
        {
            ...
            var suppliesValidatorResult = new SuppliesValidator().Validate(createdsupplies);
        ...
        private static Supply CreateSupply(Match matchValue)
        {
            ...
            var supplyCount = 1;
            double? supplyCondition = null;
            double? supplyProbability = null;
            ...
            if (!string.IsNullOrEmpty(matchValue.Groups[5].Value))
            {
                ...
                {
                    if (addon.Contains("cond") && !string.IsNullOrEmpty(addon.Substring(addon.IndexOf('=') + 1)))
                    {
                        supplyCondition = Convert.ToDouble(addon.Substring(addon.IndexOf('=') + 1));
                        continue;
                    }
                    if (addon.Contains("prob") && !string.IsNullOrEmpty(addon.Substring(addon.IndexOf('=') + 1)))
                    {
            ...
            var createdsupply = new Supply(supplyName, supplyAddons, supplyProbability, supplyCondition, supplyCount);
        ...
        private static string CreateSuppliesString(Character character)
        {
            ...
            foreach (var item in character.Supplies.Items)
            {
                ...
                stringBuilderSupply.AppendLine($"\t {item.Name} = {item.Count}");
                if (item.Addons.Count != 0) stringBuilderSupply.AppendLine($", {string.Join(", ", item.Addons)}");
                if (item.Probability is not null) stringBuilderSupply.AppendLine($", {item.Probability} ");
                if (item.Condition is not null) stringBuilderSupply.AppendLine($", {item.Condition} ");
